...
/* =============== CONTRACT ============== */
const CONTRACT_ADDRESS = "0x229D336624b807489CcB034Be5F8c967205c1C1a"; // <-- paste your GMRegistry address
const ABI = [
...
const fmtIRYS = (wei) => { try { return Number(ethers.formatEther(wei)).toFixed(3); } catch { return "0.000"; } };
...
export default function App() {
  ...
  const connect = async () => {
    ...
    if (Number(net.chainId) !== IRYS_CHAIN_ID) {
      try {
        await window.ethereum.request({
          method: "wallet_switchEthereumChain",
          params: [{ chainId: IRYS_CHAIN_HEX }],
        });
      } catch (e) {
    ...
    setSigner(s);
    setAddress(addr);
    setNetworkOk(Number(net2.chainId) === IRYS_CHAIN_ID);

    const bal = await prov2.getBalance(addr);
  ...
  const submitHirys = async () => {
    ...
    setTxHash("");
    try {
      // calling sayGM on the registry (we're using "Hirys" as the concept/message)
      const tx = await contract.sayGM(hirysMsg || "Hirys Datapunks", "");
    ...
    <div style={{ minHeight: "100vh", ...spaceBg, color: "white", fontFamily: "'Space Mono', monospace" }}>
      <style>{`
        @font-face {
          ...
          src: url('https://irys.xyz/_next/static/media/7b3a157484479c5b-s.p.woff2') format('woff2');
        }
        @font-face {
          font-family: 'Mona Sans';
          src: url('https://irys.xyz/_next/static/media/99f63174c49a6c04-s.p.woff2') format('woff2');
        }
        @keyframes fadeInUp {
      ...
      <main style={mainWrap}>
        <div style={card} className="cardAnimated">
          ...
          <button onClick={submitHirys} disabled={!address || busy} style={primaryBtn} className="primaryBtn">
...
const appIcon = {
  ...
  fontWeight: 900,
  background: "linear-gradient(135deg, rgba(28,242,200,0.25), rgba(23,194,166,0.25))",
  border: "1px solid rgba(28,242,200,0.35)"
};
...
const primaryBtn = {
  ...
  padding: "14px 16px",
  borderRadius: 14,
  border: "1px solid rgba(133,255,229,0.30)",
  background: "linear-gradient(180deg, rgba(133,255,229,0.18), rgba(133,255,229,0.10))",
  color: "white",
  fontWeight: 800,
  cursor: "pointer",
  letterSpacing: 0.3,
...
const pill = {
  padding: "6px 10px",
  borderRadius: 999,
  background: "rgba(133,255,229,0.14)",
  border: "1px solid rgba(133,255,229,0.35)",
  color: "#cffff4",
  fontWeight: 700,
  fontSize: 12
};
